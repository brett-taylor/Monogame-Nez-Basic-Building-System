...
namespace HospitalCeo.Appearance_Builder
{
    public class WorkerAppearence : Appearance
    {
        ...
        protected override Bodypart CreateBody()
        {
            ...
            return new Bodypart(bodyTextures[0], bodyTextures[1], bodyTextures[2], true, colorList[Nez.Random.range(0, colorList.Length)], false, Color.Black, true, bodyReplaceBlue[Nez.Random.range(0, bodyReplaceBlue.Length)], 11, 10, 11);
        }

        protected override Bodypart CreateHead()
        {
            Subtexture[] headTextures = GetHeadTextures();
            return new Bodypart(headTextures[0], headTextures[1], headTextures[2], true, skinColors[Nez.Random.range(0, skinColors.Length)], true, eyeColour[Nez.Random.range(0, eyeColour.Length)], true, professionalhairColours[Nez.Random.range(0, professionalhairColours.Length)], 10, 11, 10);
        }

        protected Bodypart CreateHelmet()
        {
            return new Bodypart(hardHat[0], hardHat[1], hardHat[2], true, hardHatColor[Nez.Random.range(0, hardHatColor.Length)], false, Color.Black, false, Color.Black, 9, 9, 9);
        }
